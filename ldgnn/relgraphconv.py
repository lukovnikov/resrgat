...
from ldgnn.cells import xavier_uniform_for_relmats_, RELMATINITMULT
...
class RelGraphConv(nn.Module):
    ...
    def __init__(self,
        ...
        self.rdim = rdim if rdim is not None else self.out_feat
        self.aggr = aggr

        if regularizer == "simple":
            self.weight = torch.nn.Parameter(torch.randn(self.num_rels, self.rdim, self.rdim))
            if self.rdim != self.in_feat:
                self.relproj_in = torch.nn.Linear(self.in_feat, self.rdim, bias=False)
                self.relproj_out = torch.nn.Linear(self.rdim, self.out_feat, bias=False)
            # nn.init.uniform_(self.weight, -0.1, 0.1)
            # nn.init.xavier_uniform_(self.weight, gain=nn.init.calculate_gain('relu'))
            xavier_uniform_for_relmats_(self.weight, gain=RELMATINITMULT * torch.nn.init.calculate_gain('relu'))
            self.message_func = self.simple_message_func
    ...
    def simple_message_func(self, edges):
        hs = edges.src["h"]
        _hs = hs
        relmats = self.weight[edges.data["type"]]
        if self.rdim != self.in_feat:
            hs = self.relproj_in(hs)

        hs = torch.einsum("bh,bhd->bd", hs, relmats)

        if self.rdim != self.out_feat:
            hs = self.relproj_out(hs)
            msg = hs + _hs          # equivalent to decomposition X = A Z B + I
        else:
            msg = hs
    ...
    def forward(self, g, x, etypes, norm=None):     # TODO use the norms (automatically compute?!) and set aggregator to "sum" !!!
        """ Forward computation
        ...
        # message passing
        if self.aggr == "mean":
            aggr_fn = fn.mean(msg='msg', out='h')
        elif self.aggr == "sum":
            aggr_fn = fn.sum(msg='msg', out='h')
        elif self.aggr == "max":
            aggr_fn = fn.max(msg='msg', out='h')
        else:
            raise Exception(f"unknown aggr: '{self.aggr}'")
        g.update_all(self.message_func, aggr_fn)
